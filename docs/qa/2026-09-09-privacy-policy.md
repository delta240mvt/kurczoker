...
- Finalny czysty build z commita `dab9f7ac0594afa268233725ea86b814597496ec`: preview `944f1749.kurczoker-makeover.pages.dev` i produkcja `1fc69db1.kurczoker-makeover.pages.dev`. Preview oraz `https://kurczoker.com` przeszły pełną kontrolę 64 plików: HTTP 200, MIME, cache, raport wydania i sumy SHA-256 zgodne.
- Apex i www, ścieżki polityki z końcowym ukośnikiem i bez niego: HTTP 200, `no-transform`, HTML identyczny z buildem, zwykły `mailto:hello@frinter.app`, brak wstrzykniętego dekodera e-mail. Finalny HTML polityki nie zmienił się względem sprawdzonego widoku; zmiana obejmuje nagłówki i raport wydania.
- Produkcyjna polityka sprawdzona w Codexie także na 360×800 (scrollWidth 345, viewport 360) i desktopie. Link w produkcyjnym menu obecny. Obie gałęzie `origin/main` oraz `origin/baza080926-makeover` zawierają finalny kod. Niniejsze uzupełnienie QA jest dokumentacją następującą po przetestowanym commicie źródłowym.
