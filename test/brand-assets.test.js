...
test('five roles have real geometry and eight distinct decoded animation clips',async()=>{
 ...
 assert.equal(manifest.schemaVersion,2);assert.deepEqual(manifest.assets.filter(a=>a.kind==='model').map(a=>a.id),['hero','shooter','grenadier','rusher','boss']);
