...
export function ExpeditionApp(){
 ...
 function menu(){state.menu();state.inspectSave()}
 ...
 let content;
 if(request)content=<BrandGame key={request.serial} request={request} onCheckpoint={state.checkpoint} onOutcome={state.mode==='expedition'?state.finishEncounter:undefined} onMenu={menu} onRematch={()=>state.startQuick(request.options.mapId,newSeed())}/>;
 ...
 }else content=<div className="brand-entry"><nav aria-label="Tryb gry"><button aria-pressed={page==='quick'} onClick={()=>setPage('quick')}>Szybka potyczka</button><button aria-pressed={page==='expedition'} onClick={()=>setPage('expedition')}>Wyprawa</button>{state.savedAvailable&&<button disabled={state.loadingSave} onClick={state.resume}>Wznów wyprawę</button>}<a href="/">Strona gry</a></nav>
 ...
 return <>{content}{state.mode==='expedition'&&<div className={'brand-save-status'+(state.saveError?' is-error':'')} role={state.saveError?'alert':'status'}>{saveMessage}</div>}
  ...
  {confirm&&<div className="brand-modal"><section role="dialog" aria-modal="true" aria-label="Nowa wyprawa"><h2>Zacząć od początku?</h2><p>Nowa wyprawa zastąpi aktualny zapis postępu.</p><button className="brand-primary" onClick={()=>{setConfirm(false);state.startExpedition(newSeed())}}>Tak, nowa wyprawa</button><button onClick={()=>setConfirm(false)}>Zachowaj postęp</button></section></div>}
 </>;
}
