...
export function BrandGame({request,onCheckpoint,onOutcome,onMenu,onRematch}) {
 ...
 return <main className="brand-game battle-screen" aria-label="Arena KURCZOKER" data-map-id={map.id} data-actors={JSON.stringify(snapshot?.actors??[])} data-battle-phase={snapshot?.phase??'loading'} data-sim-time={snapshot?.time} data-player-x={snapshot?.player.x} data-player-y={snapshot?.player.y} data-terrain-revision={sim?.terrain.revision??0} data-rope={snapshot?.rope?'attached':''} data-turn={snapshot?.turn} data-projectile-count={snapshot?.projectiles.length??0} data-mine-count={snapshot?.mines.length??0} data-selected-weapon={snapshot?.selectedWeaponId} data-performance={performance?JSON.stringify(performance):''}>
  <GameBoundary key={retry}><Canvas frameloop={paused||snapshot?.outcome?'demand':'always'} orthographic camera={{position:[6,5,40],zoom:70,near:.1,far:160}} dpr={1} gl={{antialias:false,alpha:false,powerPreference:'high-performance'}} fallback={<div role="alert">Ta przeglądarka nie udostępnia WebGL2.</div>}>
   <Suspense fallback={null}>{sim&&<GameRuntime key={sim.options.encounterId} sim={sim} quality="low" view={view} toolId={toolId} onView={setView} onCommand={command} onSnapshot={next=>{setSnapshot(next);if(next.phase==='player'&&next.turn!==savedTurn.current){savedTurn.current=next.turn;onCheckpoint?.(runtime.current.snapshot());}}} onEvent={onEvent} onReady={onReady} onOutcome={result=>onOutcome?.(result)}/>}</Suspense><Performance publish={setPerformance}/>
  </Canvas></GameBoundary>
