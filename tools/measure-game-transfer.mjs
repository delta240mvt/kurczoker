...
export async function measureGameTransfer(url){
 ...
 try{const page=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true}),client=await page.context().newCDPSession(page),requests=[],errors=[];let encodedBytes=0;
  ...
  const started=Date.now();await page.goto(new URL('/gra?mode=quick',url).href);await page.getByRole('button',{name:'Rozpocznij potyczkę',exact:true}).click();await page.getByRole('button',{name:'Skok',exact:true}).waitFor();const startupMs=Date.now()-started;
